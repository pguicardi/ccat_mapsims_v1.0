...
def log_likelihood(v_par, M_A, M_noise, M_std, nu, ell_c,w_SO, auto_only = auto_only, f=1.):
    ...
    if auto_only==False:
        ...
        coeff_vec = np.append(np.ones(8)*f,np.ones(4),axis=0)
        coeff_vec *= len(tmp)/np.sum(coeff_vec)
...
remove_SO = None
remove_CCAT=None
bands_SO = get_bands(chs_SO,noise_SO)
bands_CCAT = get_bands(chs_CCAT,noise_CCAT)
...
hitmaps_CCAT = noise_CCAT.get_hitmaps(tube = chs_CCAT[0][5:])
w_1_CCAT = np.mean(hitmaps_CCAT[0])

pspec_list= np.append(get_noise_PS_list(chs_SO, noise_SO, p=1, remove=remove_SO)*w_1,
                        get_noise_PS_list(chs_CCAT, noise_CCAT, p=1, remove=remove_CCAT)*w_1_CCAT, axis=0)
...
flat_samples_SO_only = np.loadtxt('SO_MCMC_output_12/flat_samples.txt', delimiter = ',' )
...
for i in np.arange(BB_avg.shape[0]):
    ...
    plt.plot(ell_bin, binned_noise[i,i]+cl[i,i]*w_SO,'--')
