...
class SOLatType:
    ...
    def get_survey_spread(self, f_sky, units='arcmin2'):
        ...
        A = f_sky * 4*np.pi #### PEDRO - Changed to uK 
        if units == 'arcmin2':
            A *= (60*180/np.pi)**2
        elif units != 'sr':
...
class CcatLatv2b(SOLatType):
    ...
    def __init__(self, sensitivity_mode=None,
        ...
        # Define the instrument.
        self.bands = np.array([
            222., 280., 348., 405., 850.])  #************************************ central frequency
        # scaled beam for 410 GHz but need to check 350 and 410.
        self.beams = np.array([
            59/60., 47/60., 37/60., 32/60., 15/60.]) #**************************** fwhm
        self.n_bands = len(self.bands)
        ...
        self.tube_configs = {
            ...
            'HF5': nar([      0,   0,    0,    0, 305400.7]),#*********************************** Sensitivities of each kind of optics tube, in uK rtsec
        }
