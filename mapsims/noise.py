...
from ccat_models import ccat_noise as ccat_models
from ccat_models import make_ccat_hitmap as ccat_hitmap
from ad_fns import *
...
sensitivity_modes = {"threshold":0,"baseline": 1, "goal": 2}
...
class BaseNoiseSimulator:
    ...
    def get_noise_properties(
        ...
        if atmosphere:
            ...
            ps_T[: self.channel_per_tube] = (
                ps_T[: self.channel_per_tube]  * nsplits * wnoise_scale * fsky[:, None]
            )
            counter = 0
            ...
            ps_P[: self.channel_per_tube] = (
                ps_P[: self.channel_per_tube]  * nsplits * wnoise_scale * fsky[:, None]
            )
            counter = 0
            for i in range(self.channel_per_tube):
                for j in range(i):
    ...
    def _process_hitmaps(self, hitmaps):
        ...
        if self.boolean_sky_fraction:
            ...
            for i in range(nhitmaps):
                hitmaps[i] = hitmaps[i]/np.max(hitmaps[i])
    ...
    def get_hitmaps(self, tube=None, hitmap=None):
        ...
        """
        if tube[:2]=="LC":
            m= ccat_hitmap.make_hitmap_ccat(self.nside)
            fsky = np.mean(m)
            return m,fsky
        else:
            ...
            return self._process_hitmaps(np.asarray(hitmaps).astype(float))
    ...
    def _get_requested_hitmaps(self, tube, hitmap):
        if tube[:2] == "LC":
            m,fsky = self.get_hitmaps(tube)
            return np.ones(2)*fsky, np.array([m,m])
...
#         if tube[:2]=="LC":
#             hitmap = ccat_hitmap.make_hitmap_ccat(self.nside)
...
class SONoiseSimulator(BaseNoiseSimulator):
    ...
    def get_survey(self, tube):
        ...
        if telescope == "SA":
            ...
            with np.errstate(divide="ignore", invalid="ignore"):
                if self.LA_noise_model[0:4] == "Ccat":
                    survey = getattr(ccat_models, self.LA_noise_model)(
                        sensitivity_mode=self.sensitivity_mode,
                        survey_efficiency=self.survey_efficiency,
#                        survey_years=self.LA_years,   #ignore survey years for now and leave it up to the choice of LA_noise_model
                        N_tubes=np.ones(count_ccat_detectors()),
                        el=self.elevation,
                    )
